...
OLD_TDB_RESPONSE=$(curl -s -H "Authorization: token $GITHUB_TOKEN" $GITHUB_API)
...
do
  if test -n "$(find ./sql/old/4.3.4/auth/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    continue
  else
...
for((counter=0;;counter++))
do
  if test -n "$(find ./sql/old/4.3.4/characters/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    continue
  else
...
for((counter=0;;counter++))
do
  if test -n "$(find ./sql/old/4.3.4/world/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    continue
  else
...
for((counter=0;;counter++))
do
  if test -n "$(find ./sql/old/4.3.4/hotfixes/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    continue
  else
